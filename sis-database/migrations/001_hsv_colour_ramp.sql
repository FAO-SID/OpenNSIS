-- ============================================================================
-- 001_hsv_colour_ramp.sql
--
-- Colour-ramp triggers: interpolate through HSV (hue) instead of linear RGB so
-- green→red runs through yellow/orange, not muddy brown; and fix soil_data.map()
-- to JOIN the layer's actual mapped_property (the old form pulled an arbitrary
-- property's colours). Adds the soil_data._ramp_color() helper.
--
-- Idempotent (CREATE OR REPLACE) — safe to re-run; already present in fresh
-- dumps, this converges installs seeded from an older one.
-- ============================================================================

CREATE OR REPLACE FUNCTION soil_data._ramp_color(start_hex text, end_hex text, n integer, i integer)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE
AS $function$
...
CREATE OR REPLACE FUNCTION soil_data.map()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
DECLARE
