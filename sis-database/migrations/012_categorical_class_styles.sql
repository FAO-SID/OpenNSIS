-- 012: categorical rasters render from their class table.
--
-- soil_data.map() painted every raster with the property's HSV colour ramp,
-- including categorical ones — where each class value has its own colour in
-- soil_data.class (the rows the SLD and the web legend already use). Emit one
-- flat-colour STYLE per published class value for categorical properties, so
-- the WMS render, the SLD, the legend and the admin colour editor all agree.
-- Quantitative properties keep the interval ramp; categorical mapsets without
-- class rows (or with more than 60) keep the ramp fallback.
